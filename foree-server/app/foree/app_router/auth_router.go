...
func NewAuthRouter(authService *foree_auth_service.AuthService) *AuthRouter {
	return &AuthRouter{
...
func (c *AuthRouter) RegisterRouter(router *mux.Router) {
	...
	// Get user extra
	router.HandleFunc("/user_extra", simpleGetWrapper(c.authService.GetUserExtra)).Methods("GET")
	// Update passwd
